...
declare global {
    namespace Express {
        interface Request {
            'authentication': { 'id'?: string, 'username'?: string, 'languageId'?: number, 'countryId'?: number, 'permissionLevel'?: number};
            'models'?: any;
        }
        interface Response {
            'startTime'?: number;
...
class App {
    ...
    private implementToken(): void {
        this.express.use(jwt({ 'secret': baseConfig.secretKeyHash, 'requestProperty': 'authentication' }).unless({'path': [
            /\/documentation*/,
            /\/v1\/auth*/
        ]}));
    ...
    private implementCache(): void {
        if (this.env === 'production') {
            this.cache = redisCache({
                ...
                'auth_pass': baseConfig.cache[this.env].password.password,
