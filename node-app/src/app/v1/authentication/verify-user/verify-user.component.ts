...
export class VerifyUser extends CoreInterface {
    ...
    verify(req: Request, res: Response): void {
        ...
        return req.models.users.update({ 'verified': true }, whereData)
            .then(
                (user: number[]) => (user[0]) ? this.response.success(res, 'verify') : this.response.failed(res, 'verify')
            )
            .catch(
                (error) => this.response.failed(res, 'verify', error)
            );
    }
