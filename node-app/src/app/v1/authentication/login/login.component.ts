...
export class LogIn extends CoreMiddleware {
    ...
    login(req: Request, res: Response): void {
        ...
        const roleIds: Array<any> = (req.query.roleId) ? (req.query.roleId).toString().split(',').map( i => Number(i)) : [];
        ...
        return req.models.users.findOne(whereData)
            ...
            .then(
                (user: UsersAttributes) => {
                    if (!user) {
                        return this.updateLoginAttempt(req.models.users, loginInfo.id, loginInfo.loginAttempt)
                            .then(
                                (data) => this.response.failed(res, 'login', '')
                            );
                    }

                    const cleanUser: UsersAttributes = this.helper.cleanSequelizeData(user);
                    return this.updateLastLogIn(req.models.users, cleanUser.id, cleanUser.loginCount)
                        .then(
                            // permission level hardcoded to 1 first
                            (data) => this.response.success(res, 'login', this.helper.createJwtToken({...cleanUser, 'permissionLevel': 1}))
                        );
                }
            )
            .catch(
                (error) => this.response.failed(res, 'login', error)
            );
    }
