...
const authKey: string = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpZCI6ImRhNzQ0YTdlLWMwNTgtNDA4Mi1iYTNlLWQ1MmYzNjI5MzY5OCIsInVzZXJuYW1lIjoic3VwZXJBZG1pbiIsImZpcnN0TmFtZSI6IlN1cGVyIiwibGFzdE5hbWUiOiJBZG1pbiIsInJvbGVJZCI6MSwibG9naW5Db3VudCI6MCwicGVybWlzc2lvbkxldmVsIjoxfQ.IsTx_7ru52zlzk8af1mcSUh2Atsnv677dumVbnRptpU';
...
describe('users component', () => {
    ...
    it('should fetch post and put information', (done) => {
        ...
        expect(application.autoFillPostInformation(dataPassword, auth)).to.contain.keys('salt', 'password', 'passwordExpiry', 'roleId');

        expect(application.autoFillPutInformation(data, auth)).to.eql({
            'firstName': 'test',
            'updatedUserId': 'abc'
        });
        expect(application.autoFillPutInformation(dataPassword, auth)).to.contain.keys('salt', 'password', 'passwordExpiry');

        done();
    });

    it('should get all user', (done) => {
        supertest(app).get('/v1/core/users?test=true')
    ...
    it('should create user', (done) => {
        supertest(app).post('/v1/core/user?test=true')
            .set('Authorization', `Bearer ${authKey}`)
            .send({
                'firstName': 'test',
                'lastName': 'test',
    ...
    it('should get user', (done) => {
        supertest(app).get(`/v1/core/user/${testName}?test=true`)
            .set('Authorization', `Bearer ${authKey}`)
            .expect('Content-Type', /json/)
            .expect(200, (err, res) => {
                if (err) { return done(err); }
    ...
    it('should update user', (done) => {
        supertest(app).put(`/v1/core/user/${testName}?test=true`)
            .set('Authorization', `Bearer ${authKey}`)
            .send({
    ...
    it('should delete user', (done) => {
        supertest(app).delete(`/v1/core/user/${testName}?test=true`)
            .set('Authorization', `Bearer ${authKey}`)
            .expect('Content-Type', /json/)
            .expect(200, (err, res) => {
                if (err) { return done(err); }
