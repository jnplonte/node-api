...
export class Languages extends CoreInterface {
    pageCache: any;
    ...
    all(req: Request, res: Response): void {
        ...
        return this.query.getAll(req.models.languages, whereData, req.query)
            .then(
                (languages: any) => {
     ...
     *
     * @apiParam (url segment) {Number} id language id
     * @apiParam (url parameter) {String} key key search Ex. ?key=name
     */
    get(req: Request, res: Response): void {
        const whereData = {};

        return this.query.getOne(req.models.languages, whereData, req.query.key, req.params.id)
            .then(
                (language: LanguagesAttributes) => this.response.success(res, 'get', language || {})
            )
            .catch(
                (error) => this.response.failed(res, 'get', error)
            );
    ...
    post(req: Request, res: Response): void {
        ...
        return this.query.post(req.models.languages, data)
            .then(
                (language: LanguagesAttributes) => this.cacheCheck(this.pageCache, this.nameCache)((language) ? this.response.success(res, 'post', language.id) : this.response.failed(res, 'post'))
            )
            .catch(
    ...
    put(req: Request, res: Response): void {
        ...
        return this.query.put(req.models.languages, data, whereData)
            .then(
                (language: number[]) => this.cacheCheck(this.pageCache, this.nameCache)((language[0]) ? this.response.success(res, 'put', id) : this.response.failed(res, 'put'))
            )
            .catch(
    ...
    delete(req: Request, res: Response): void {
        ...
        return this.query.delete(req.models.languages, whereData)
            .then(
                (language: number) => this.cacheCheck(this.pageCache, this.nameCache)((language) ? this.response.success(res, 'delete', id) : this.response.failed(res, 'delete'))
            )
            .catch(
